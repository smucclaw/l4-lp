...
py_transform_compound_term(InDict, OutTaggedDict), is_dict(InDict) =>
  ...
  OutTaggedDict = OutDict.
  % OutTaggedDict = py{'TYPE':dict, 'DICT':OutDict}.

py_transform_compound_term(InCompoundTerm, OutTaggedDict) =>
  InCompoundTerm =.. [Functor | Args],
  maplist(py_transform_compound_term, Args, OutArgs),
  dict_pairs(OutTaggedDict, py, ['$t'-t, functor-Functor, Functor-OutArgs]).
